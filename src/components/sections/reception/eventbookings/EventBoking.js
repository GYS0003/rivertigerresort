...
const EventBoking = () => {
  ...
  const [filters, setFilters] = useState({
    eventDate: new Date().toISOString().split('T')[0], // Auto-select today's date
    searchQuery: '',
    paymentStatus: 'all',
  });
  ...
  const filteredBookings = bookings.filter(booking => {
    ...
    // Search filtering (ID, email, or event name)
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      const matchesId = booking._id.toLowerCase().includes(query);
      const matchesEmail = booking.userEmail.toLowerCase().includes(query);
      const matchesEvent = booking.items.title.toLowerCase().includes(query);
      const matchesDate = bookingEventDate ? 
        bookingEventDate.toLocaleDateString('en-IN').includes(query) : false;

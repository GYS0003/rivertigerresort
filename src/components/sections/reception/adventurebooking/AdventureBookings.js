...
import { FiSearch, FiCalendar, FiX, FiUser, FiCheckCircle, FiXCircle, FiClock, FiChevronDown, FiChevronUp } from 'react-icons/fi';
...
const AdventureBookings = () => {
  ...
  const [filters, setFilters] = useState({
    adventureDate: new Date().toISOString().split('T')[0], // Auto-select today's date
    searchQuery: '',
    paymentStatus: 'all',
  });
  ...
  const fetchAdventureBookings = async () => {
    try {
      const res = await fetch('/api/adventure/bookings', {
        method: 'GET',
      });
  ...
  const clearFilters = () => {
    setFilters({
      adventureDate: new Date().toISOString().split('T')[0], // Reset to today's date
      searchQuery: '',
      paymentStatus: 'all',
    });
  };
...
const filteredBookings = bookings.filter(booking => {
  const { adventureDate, searchQuery, paymentStatus } = filters;
  const bookingDate = new Date(booking.adventureDate);
  
  // Date filtering - exact match with selected date
  if (adventureDate) {
    const selectedDate = new Date(adventureDate);
  ...
  if (searchQuery) {
    ...
    // Check activity names with comprehensive null checks
    const matchesActivity = booking.items && Array.isArray(booking.items) 
      ? booking.items.some(item => item && item.name && item.name.toLowerCase().includes(query))
      : false;
    
    // Check formatted date with null check
    const matchesDate = booking.adventureDate 
      ? new Date(booking.adventureDate).toLocaleDateString('en-IN').toLowerCase().includes(query)
      : false;
    
    if (!(matchesId || matchesEmail || matchesActivity || matchesDate)) {
      return false;
    }
  }
  ...
  const calculateTotalGuests = (items) => {
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1 hidden sm:block">
            Manage all adventure reservations in one place
          </p>
        </div>
      ...
      <div className={`bg-white rounded-xl shadow-sm border p-4 sm:p-6 mb-6 ${isMobile && !isFiltersOpen ? 'hidden' : ''}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="sm:col-span-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="ID, email, activity, or date..."
          ...
          <div>
            <label htmlFor="adventureDate" className="block text-sm font-medium text-gray-700 mb-1">
              Adventure Date
            </label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-1 lg:px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    Booking ID
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredBookings.map((booking) => (
                  <tr key={booking._id} className="hover:bg-gray-50">
                    <td className="px-1 lg:px-2 py-4">
                    ...
                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {formatDate(booking.adventureDate)}
                    </td>
                    <td className="px-1 lg:px-2 py-4">
                      <div className="text-sm text-gray-900 max-w-xs">
                        {booking.items.map((item, index) => (
                          <div key={item._id} className="flex justify-between mb-1 last:mb-0">
                ...
                <div className="mt-3">
                  ...
                  <div className="space-y-2">
                    {booking.items.map(item => (
                      <div key={item._id} className="flex justify-between text-sm bg-gray-50 rounded-lg p-2">
                        <div className="truncate pr-2">{item.name}</div>
                        <div className="whitespace-nowrap">
                          {item.count} × {formatCurrency(item.pricePerPerson)}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-sm">
                        {calculateTotalGuests(booking.items)}
                      </div>
                    </div>
...
export default AdventureBookings;

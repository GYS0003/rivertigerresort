/* components/login/LoginSignupModal.jsx */
'use client';
...
/* ---------- modal component ---------- */
export default function LoginSignupModal({ isOpen, onClose, handleBooking }) {
  const router = useRouter();

  /* 1. ALL hooks live here – never inside conditions */
  ...
  async function handleVerifyOtp(e) {
    ...
    try {
      ...
      onClose();                 // hide login modal
      if (handleBooking) handleBooking();  // resume parent flow (event booking)
      router.refresh?.();        // optional – revalidate layouts
    } catch (err) {
